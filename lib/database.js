...
class Database {
  ...
  insertPing (ip, timestamp, unsafePlayerCount) {
    if (unsafePlayerCount === -1) {
      logger.warn('%s 在 %s 的POST请求空返回', ip, timestamp)
      return
    }
  ...
  _insertPingTo (ip, timestamp, unsafePlayerCount, db) {
    db.query('INSERT INTO netease_online (`time`, `server`, `online`) VALUES (?, ?, ?)', [timestamp, ip, unsafePlayerCount], (err, result) => {
      if (err) {
