...
class TimeTracker {
  ...
  newPointTimestamp (timestamp) {
    const second = TimeTracker.toSeconds(timestamp)
    if (!(this._serverGraphPoints.includes(second))) {
      TimeTracker.pushAndShift(this._serverGraphPoints, second, TimeTracker.getMaxServerGraphDataLength())
    }
  ...
  getServerGraphPoints () {
    return this._serverGraphPoints
  }
