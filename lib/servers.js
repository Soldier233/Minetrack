...
class ServerRegistration {
  ...
  getPingHistory () {
    if (this.graphData.length > 0) {
      ...
      payload.playerCount = this.graphData[this.graphData.length - 1]
      ...
      const history = []
      for (let i = Math.min(this.graphData.length - TimeTracker.getMaxServerGraphDataLength(), 0); i < this.graphData.length; i++) {
        history.push(this.graphData[i])
      }
      payload.playerCountHistory = history
